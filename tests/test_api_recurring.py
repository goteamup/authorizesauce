...
class RecurringAPITests(TestCase):
    def setUp(self):
        self.patcher = mock.patch("authorize.apis.recurring.Client")
    ...
    def test_create_subscription(self):
        ...
        subscription_id = self.api.create_subscription(
        ...
        self.assertEqual(subscription.payment.creditCard._kind, "CreditCardType")
        self.assertEqual(subscription.payment.creditCard.cardNumber, "4111111111111111")
        self.assertEqual(
            subscription.payment.creditCard.expirationDate, "{0}-01".format(year)
        )
        ...
        self.api.create_subscription(credit_card, 10, start, days=14, occurrences=10)
        subscription = service.call_args[0][1]
    ...
    def test_update_subscription(self):
        ...
        past_start = date.today() - timedelta(days=1)
        self.assertRaises(
            AuthorizeInvalidError, self.api.update_subscription, "1", start=past_start
        )
        ...
        self.assertTrue(
            isinstance(subscription.paymentSchedule.totalOccurrences, mock.Mock)
        )
        self.assertTrue(isinstance(subscription.trialAmount, mock.Mock))
        self.assertTrue(
            isinstance(subscription.paymentSchedule.trialOccurrences, mock.Mock)
        )
