...
class ClientTests(TestCase):
    ...
    def test_basic_authorize_client(self):
        ...
        self.assertEqual(
            self.customer_api.call_args, (("123", "456", False, False), {})
        )
        self.assertEqual(
            self.recurring_api.call_args, (("123", "456", False, False), {})
        )

    def test_authorize_client_payment_creators(self):
        self.assertTrue(
            isinstance(self.client.card(self.credit_card), AuthorizeCreditCard)
        )
        self.assertTrue(
            isinstance(
                self.client.card(self.credit_card, self.address), AuthorizeCreditCard
            )
        )
        self.assertTrue(
            isinstance(self.client.transaction("123"), AuthorizeTransaction)
        )
        self.assertTrue(
            isinstance(self.client.saved_card("123|456"), AuthorizeSavedCard)
        )
        self.assertTrue(isinstance(self.client.recurring("123"), AuthorizeRecurring))
