...
class RecurringAPI(object):
    ...
    def client_auth(self):
        if not hasattr(self, "_client_auth"):
            self._client_auth = self.client.factory.create("MerchantAuthenticationType")
    ...
    def create_subscription(
        ...
        if (days and months) or not (days or months):
            raise AuthorizeInvalidError(
                "Please provide either the months or "
        ...
        if days:
            try:
                ...
                raise AuthorizeInvalidError(
                    "The interval days must be an " "integer value between 7 and 365."
                )
        ...
        if trial_amount and trial_occurrences:
            ...
            raise AuthorizeInvalidError(
                "To indicate a trial period, you "
