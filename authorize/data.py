...
class CreditCard(object):
    ...
    def validate(self):
        ...
        if (
            sum(num[::-2] + list(map(lambda d: sum(divmod(d * 2, 10)), num[-2::-2])))
            % 10
        ):
            raise AuthorizeInvalidError("Credit card number is not valid.")
...
class Address(object):
    ...
    def __init__(self, street=None, city=None, state=None, zip_code=None, country="US"):
        self.street = street
        self.city = city
        self.state = state
    ...
    def __repr__(self):
        return "<Address {0.street}, {0.city}, {0.state} {0.zip_code}>".format(self)
